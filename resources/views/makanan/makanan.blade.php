          ...
          <div class="panel panel-default">
            <div class="panel-heading">
              ...
              <div class="panel-body">
                <table id="makanan-table" class="table table-striped">
                  <thead>
                    <tr>
                      <th width="30">No</th>
                      <th>Penyuplai</th>
...
<script src="{{ asset('assets/bootstrap/js/ie-emulation-modes-warning.js') }}"></script>
  ...
  <script type="text/javascript">
    var table = $('#makanan-table').DataTable({
      processing : true,
      serverSide : true,
    ...
    function tambah() {
      ...
      $('#makanan').modal('show');
      $('#makanan form')[0].reset();
      $('#makanan').appendTo("body").modal('show');
      $('.modal-title').text('Masukan Produk Makanan');
    }
    ...
    $(function(){
            $('#makanan form').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('makanan') }}";
                    else url = "{{ url('makanan') . '/' }}" + id;

                    $.ajax({
                        url : url,
                        type : "POST",
                        //data : $('#makanan form').serialize(),
                        data: new FormData($("#makanan form")[0]),
                        contentType: false,
                        processData: false,
                        success : function(data) {
                            $('#makanan').modal('hide');
